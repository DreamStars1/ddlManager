...
public class SessionMonitorController {
    ...
    public AjaxResult getSessionStatistics() {
        try {
            ...
            return AjaxResult.ok("获取会话统计成功", Map.of(
    ...
    public AjaxResult cleanAllSessions() {
        try {
            ...
            return AjaxResult.ok("所有Session数据清理完成", Map.of(
                    "cleanedSpringSessions", keys != null ? keys.size() : 0,
                    "cleanedUserSessions", userSessionKeys != null ? userSessionKeys.size() : 0
            ));
        } catch (Exception e) {
